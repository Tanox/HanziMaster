// src/lib/i18n/translations/zh-TW.ts v5.2.23
export const zhTW = {
  common: {
    home: '首頁',
    learn: '學習',
    practice: '練習',
    startLearning: '立即開始學習',
    exploreLibrary: '瀏覽字庫',
    dailyPractice: '每日練習',
    masterCharacters: '掌握最常用的12個漢字。',
    dayStreak: '天連續學習',
    practiceWriting: '練習書寫',
    hearPronunciation: '聽發音',
    strokeCount: '筆畫數',
    strokes: '畫',
    foreverQuote: '「永」字包含了所有8種基本筆畫。',
    learners: '學員正在學習',
    strokeMastery: '筆畫掌握度',
    theme: {
      light: '淺色',
      dark: '深色',
      system: '跟隨系統',
    },
    cancel: '取消',
    submit: '提交',
    skipToContent: '跳到主要內容',
    openMenu: '打開選單',
    closeMenu: '關閉選單',
    navigationMenu: '導覽選單',
  },
  home: {
    heroTitle: '精通中文',
    heroSubtitle: '一筆一劃',
    heroSuffix: '',
    heroDescription: '漢字大師透過引導式練習、筆順演示與沉浸式複習，助你掌握漢字書寫。',
    aiInsightsTitle: '筆畫回饋',
    aiInsightsDesc: '借助清晰的筆順指引與結構提示研習每個漢字，寫出自信、美觀的字跡。',
    etymologyTitle: '詞源與文化',
    etymologyDesc: '發現漢字背後的故事。了解它們如何從古代象形文字演變為現代形態。',
    adaptiveTitle: '自適應學習',
    adaptiveDesc: '我們的演算法適應您的節奏，專注於您覺得有挑戰性的漢字，同時鞏固您已掌握的知識。',
    featuresTitle: '為什麼選擇漢字大師',
    featuresSubtitle: '透過引導式練習與沉浸式複習學習漢字，功能專為加速您的學習進度而設計。',
    aiBadge: '互動學習',
    yongCharacterTitle: '永字八法',
    yongCharacterDesc: '掌握永字的八個基本筆畫，打開漢字書寫的精髓',
    yongCharacterStrokes: '筆畫掌握',
  },
  practice: {
    center: '中心',
    subtitle: '多種練習方式，全方位提升您的漢字掌握程度。',
    writingTitle: '書寫練習',
    writingDesc: '在畫布上練習書寫，獲得清晰的筆順指引與結構提示。',
    quizTitle: '記憶測驗',
    quizDesc: '趣味測驗鞏固漢字含義與讀音，強化記憶效果。',
    progressTitle: '學習進度',
    progressDesc: '詳細資料分析，了解您的學習軌跡與成長。',
    startNow: '立即開始',
    weeklyProgress: '本週學習進度',
    mon: '週一',
    tue: '週二',
    wed: '週三',
    thu: '週四',
    fri: '週五',
    sat: '週六',
    sun: '週日',
    today: '今',
    pending: '待學',
    charactersLearned: '已學漢字',
    dayStreak: '連續學習天數',
    accuracy: '正確率',
    writingDialogDesc: '在田字格中練習書寫漢字',
    characterProgress: '字符 {{current}} / {{total}}',
    clear: '清除',
    next: '下一個',
    done: '完成',
    quizDialogDesc: '選擇正確的拼音',
    quizComplete: '測驗完成！',
    correct: '正確',
    wrong: '錯誤',
    close: '關閉',
    retake: '重新測驗',
    viewResults: '查看結果',
    listenPronunciation: '聽發音',
    nextQuestion: '下一題',
    progressDialogDesc: '你的學習進展',
    writeOnCanvas: '在畫布上書寫漢字',
    question: '問題',
    learnedCharacters: '已學漢字',
    dayStreakLabel: '連續天數',
    accuracyLabel: '準確率',
    studyCharacters: '學習漢字',
    strokeCount: '筆畫數：{{count}}',
  },
  learn: {
    one: '一',
    two: '二',
    three: '三',
    person: '人',
    big: '大',
    small: '小',
    mouth: '口',
    sunDay: '日',
    moonMonth: '月',
    mountain: '山',
    water: '水',
    fire: '火',
    radical: '部首',
    structure: '結構',
    independent: '獨體',
    wordsTitle: '詞組',
    exampleTitle: '例句',
    showStrokeOrder: '顯示筆順',
    hideStrokeOrder: '隱藏筆順',
    words: {
      oneDay: '一天',
      oneItem: '一個',
      first: '第一',
      february: '二月',
      second: '第二',
      twenty: '二十',
      march: '三月',
      threeItems: '三個',
      thirty: '三十',
      people: '人們',
      adult: '大人',
      chinesePerson: '中國人',
      university: '大學',
      great: '偉大',
      child: '小孩',
      primarySchool: '小學',
      careful: '小心',
      saliva: '口水',
      entrance: '入口',
      exit: '出口',
      days: '日子',
      tomorrow: '明日',
      japan: '日本',
      moon: '月亮',
      month: '月份',
      firstMonth: '正月',
      mountainWater: '山水',
      climbMountain: '上山',
      snowMountain: '雪山',
      fruit: '水果',
      riverWater: '河水',
      drinkWater: '喝水',
      train: '火車',
      flame: '火苗',
      volcano: '火山',
    },
    example: {
      oneBook: '我有一本書。',
      twentyYearsOld: '他今年二十歲。',
      threeApples: '桌上有三個蘋果。',
      goodPerson: '他是一個好人。',
      bigApple: '這是一個大蘋果。',
      cuteCat: '小貓很可愛。',
      openMouth: '請張開你的口。',
      goodDay: '今天是好日子。',
      beautifulMoon: '今晚的月亮很美。',
      climbMountain: '我想去爬山。',
      glassOfWater: '請給我一杯水。',
      carefulFire: '小心火！',
    },
    pinyin: '拼音',
    meaning: '含義',
    wordExamples: '詞語與短語',
    strokeOrder: '筆順',
    strokeCount: '筆畫數：{{count}}',
    playAudio: '播放發音',
    startQuiz: '開始測驗',
    nextCharacter: '下一個字',
    quizTitle: '漢字測驗',
    quizDesc: '測試你對漢字拼音和含義的掌握。',
    quizPlaceholder: '輸入拼音…',
    quizCorrect: '正確！',
    quizIncorrect: '錯誤',
  },
  footer: {
    copyright: '© 2026 漢字大師 HanziMaster v5.2.23. 保留所有權利。',
  },
  meta: {
    title: 'HanziMaster 漢字大師',
    description: '互動式漢字學習平台',
  },
};
