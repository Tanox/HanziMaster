// src/lib/i18n/translations/de.ts v5.2.24
export const de = {
  common: {
    home: 'Startseite',
    learn: 'Lernen',
    practice: 'Übung',
    startLearning: 'Jetzt Lernen',
    exploreLibrary: 'Bibliothek Erkunden',
    dailyPractice: 'Tägliche Übung',
    masterCharacters: 'Meistere die 12 häufigsten Zeichen.',
    dayStreak: 'Tage Streak',
    practiceWriting: 'Schreiben Üben',
    hearPronunciation: 'Aussprache Hören',
    strokeCount: 'Strichanzahl',
    strokes: 'Striche',
    foreverQuote: 'Das Zeichen für "für immer" enthält alle 8 Grundstriche.',
    learners: 'Lernende lernen',
    strokeMastery: 'Strichbeherrschung',
    theme: {
      light: 'Hell',
      dark: 'Dunkel',
      system: 'System',
    },
    cancel: 'Abbrechen',
    submit: 'Senden',
    skipToContent: 'Zum Inhalt springen',
    openMenu: 'Menü öffnen',
    closeMenu: 'Menü schließen',
    navigationMenu: 'Navigationsmenü',
  },
  home: {
    heroTitle: 'Chinesisch Meistern',
    heroSubtitle: 'Ein Strich',
    heroSuffix: ' auf Einmal',
    heroDescription: 'HanziMaster hilft Ihnen, chinesische Zeichen durch geführte Übungen, Strichfolgen-Demos und immersives Wiederholen zu meistern.',
    aiInsightsTitle: 'Strich-Feedback',
    aiInsightsDesc: 'Lernen Sie jedes Zeichen mit klaren Strichfolgen-Hinweisen und Strukturhilfen für ein sicheres, schönes Schriftbild.',
    etymologyTitle: 'Etymologie & Kultur',
    etymologyDesc: 'Entdecken Sie die Geschichten hinter den Zeichen. Verstehen Sie, wie sie sich von alten Piktogrammen zu modernen Formen entwickelt haben.',
    adaptiveTitle: 'Adaptives Lernen',
    adaptiveDesc: 'Unser Algorithmus passt sich Ihrem Tempo an und konzentriert sich auf die Zeichen, die Sie herausfordernd finden, während er das stärkt, was Sie wissen.',
    featuresTitle: 'Warum HanziMaster Wählen',
    featuresSubtitle: 'Lerne chinesische Zeichen durch geführte Übung und immersives Wiederholen, mit Funktionen zum Beschleunigen deines Fortschritts.',
    aiBadge: 'Interaktives Lernen',
    yongCharacterTitle: 'Die Acht Striche von 永',
    yongCharacterDesc: 'Meistern Sie die acht Grundstriche durch das Zeichen 永 und erschließen Sie das Wesen der chinesischen Kalligrafie',
    yongCharacterStrokes: 'Strichbeherrschung',
  },
  practice: {
    center: 'Zentrum',
    subtitle: 'Mehrere Übungsmodi, um chinesische Zeichen aus allen Blickwinkeln zu meistern.',
    writingTitle: 'Schreibübung',
    writingDesc: 'Übe das Schreiben auf dem Leinwand mit klaren Strichfolgen-Hinweisen und Strukturhilfen.',
    quizTitle: 'Gedächtnisquiz',
    quizDesc: 'Lustige Quizze zur Festigung von Zeichenbedeutungen und Aussprachen.',
    progressTitle: 'Lernfortschritt',
    progressDesc: 'Detaillierte Analysen, um Ihre Lernreise und Ihr Wachstum zu verfolgen.',
    startNow: 'Jetzt Starten',
    weeklyProgress: 'Wöchentlicher Fortschritt',
    mon: 'Mo',
    tue: 'Di',
    wed: 'Mi',
    thu: 'Do',
    fri: 'Fr',
    sat: 'Sa',
    sun: 'So',
    today: 'Heute',
    pending: 'Ausstehend',
    charactersLearned: 'Gelernte Zeichen',
    dayStreak: 'Tage Streak',
    accuracy: 'Genauigkeit',
    writingDialogDesc: 'Üben Sie das Schreiben chinesischer Zeichen im Raster',
    characterProgress: 'Zeichen {{current}} / {{total}}',
    clear: 'Löschen',
    next: 'Weiter',
    done: 'Fertig',
    quizDialogDesc: 'Wählen Sie die korrekte Pinyin',
    quizComplete: 'Quiz Abgeschlossen!',
    correct: 'Richtig',
    wrong: 'Falsch',
    close: 'Schließen',
    retake: 'Erneut versuchen',
    viewResults: 'Ergebnisse anzeigen',
    listenPronunciation: 'Hören',
    nextQuestion: 'Nächste Frage',
    progressDialogDesc: 'Ihr Lernfortschritt',
    writeOnCanvas: 'Auf der Leinwand schreiben',
    question: 'Frage',
    learnedCharacters: 'Gelernte Zeichen',
    dayStreakLabel: 'Tage Streak',
    accuracyLabel: 'Genauigkeit',
    studyCharacters: 'Zeichen Lernen',
    strokeCount: 'Strichanzahl: {{count}}',
  },
  learn: {
    one: 'Eins',
    two: 'Zwei',
    three: 'Drei',
    person: 'Person',
    big: 'Groß',
    small: 'Klein',
    mouth: 'Mund',
    sunDay: 'Sonne/Tag',
    moonMonth: 'Mond/Monat',
    mountain: 'Berg',
    water: 'Wasser',
    fire: 'Feuer',
    radical: 'Radikal',
    structure: 'Struktur',
    independent: 'Unabhängig',
    wordsTitle: 'Wörter und Redewendungen',
    exampleTitle: 'Beispielsatz',
    showStrokeOrder: 'Strichfolge Anzeigen',
    hideStrokeOrder: 'Strichfolge Ausblenden',
    words: {
      oneDay: 'ein Tag',
      oneItem: 'eins (Stück)',
      first: 'erstens',
      february: 'Februar',
      second: 'zweitens',
      twenty: 'zwanzig',
      march: 'März',
      threeItems: 'drei (Stück)',
      thirty: 'dreißig',
      people: 'Leute',
      adult: 'Erwachsener',
      chinesePerson: 'Chinese',
      university: 'Universität',
      great: 'großartig',
      child: 'Kind',
      primarySchool: 'Grundschule',
      careful: 'vorsichtig',
      saliva: 'Speichel',
      entrance: 'Eingang',
      exit: 'Ausgang',
      days: 'Tage',
      tomorrow: 'morgen',
      japan: 'Japan',
      moon: 'Mond',
      month: 'Monat',
      firstMonth: 'erster Monat',
      mountainWater: 'Landschaft',
      climbMountain: 'Berg besteigen',
      snowMountain: 'Schneeberg',
      fruit: 'Frucht',
      riverWater: 'Flusswasser',
      drinkWater: 'Wasser trinken',
      train: 'Zug',
      flame: 'Flamme',
      volcano: 'Vulkan',
    },
    example: {
      oneBook: 'Ich habe ein Buch.',
      twentyYearsOld: 'Er ist dieses Jahr zwanzig Jahre alt.',
      threeApples: 'Es sind drei Äpfel auf dem Tisch.',
      goodPerson: 'Er ist ein guter Mensch.',
      bigApple: 'Das ist ein großer Apfel.',
      cuteCat: 'Das Kätzchen ist sehr süß.',
      openMouth: 'Bitte öffnen Sie den Mund.',
      goodDay: 'Heute ist ein guter Tag.',
      beautifulMoon: 'Der Mond ist heute Nacht wunderschön.',
      climbMountain: 'Ich möchte bergsteigen gehen.',
      glassOfWater: 'Bitte geben Sie mir ein Glas Wasser.',
      carefulFire: 'Vorsicht vor Feuer!',
    },
    pinyin: 'Pinyin',
    meaning: 'Bedeutung',
    wordExamples: 'Wörter & Phrasen',
    strokeOrder: 'Strichfolge',
    strokeCount: 'Strichanzahl: {{count}}',
    playAudio: 'Audio abspielen',
    startQuiz: 'Quiz starten',
    nextCharacter: 'Nächstes Zeichen',
    quizTitle: 'Zeichen-Quiz',
    quizDesc: 'Testen Sie Ihr Wissen über Pinyin und Bedeutung des Zeichens.',
    quizPlaceholder: 'Pinyin eingeben...',
    quizCorrect: 'Richtig!',
    quizIncorrect: 'Falsch',
  },
  footer: {
    copyright: '© 2026 HanziMaster 汉字大师 v5.2.24. Alle Rechte vorbehalten.',
  },
  meta: {
    title: 'HanziMaster 汉字大师',
    description: 'Interaktive Plattform zum Erlernen chinesischer Zeichen',
  },
};
