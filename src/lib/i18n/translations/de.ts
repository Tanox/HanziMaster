// src/lib/i18n/translations/de.ts v3.0.0
export const de = {
  common: {
    home: 'Startseite',
    learn: 'Lernen',
    practice: 'Übung',
    startLearning: 'Jetzt Lernen',
    exploreLibrary: 'Bibliothek Erkunden',
    dailyPractice: 'Tägliche Übung',
    masterCharacters: 'Meistere die 12 häufigsten Zeichen.',
    pronunciationNotSupported: 'Ihr Browser unterstützt die Aussprachefunktion nicht.',
    skipToContent: 'Zum Inhalt springen',
    openMenu: 'Menü öffnen',
    navigationMenu: 'Navigationsmenü',
    closeMenu: 'Menü schließen',
    changeLanguage: 'Sprache ändern',
    selectLanguage: 'Sprache auswählen',
    practiceWriting: 'Schreiben Üben',
    hearPronunciation: 'Aussprache Hören',
    strokeCount: 'Strichanzahl',
    strokes: 'Striche',
    independent: 'Unabhängig',
    completed: 'Abgeschlossen',
    theme: {
      light: 'Hell',
      dark: 'Dunkel',
      system: 'System',
    },
  },
  home: {
    heroTitle: 'Chinesisch Meistern',
    heroSubtitle: 'Ein Strich',
    heroSuffix: ' auf Einmal',
    heroDescription: 'HanziMaster nutzt fortschrittliche KI, um Sie durch die komplexe Kunst der chinesischen Zeichen zu führen. Personalisierte Lernpfade, Echtzeit-Feedback und immersives Üben.',
    aiInsightsTitle: 'KI-gestützte Einblicke',
    aiInsightsDesc: 'Unsere KI analysiert Ihre Handschrift und liefert sofortiges Feedback zu Strichreihenfolge, Balance und Ästhetik.',
    etymologyTitle: 'Etymologie & Kultur',
    etymologyDesc: 'Entdecken Sie die Geschichten hinter den Zeichen. Verstehen Sie, wie sie sich von alten Piktogrammen zu modernen Formen entwickelt haben.',
    adaptiveTitle: 'Adaptives Lernen',
    adaptiveDesc: 'Unser Algorithmus passt sich Ihrem Tempo an und konzentriert sich auf die Zeichen, die Sie herausfordernd finden, während er das stärkt, was Sie wissen.',
    featuresTitle: 'Warum HanziMaster Wählen',
    featuresSubtitle: 'Erleben Sie die Zukunft des chinesischen Zeichnenlernens mit unseren hochmodernen Funktionen, die entwickelt wurden, um Ihren Fortschritt zu beschleunigen.',
    aiBadge: 'KI-gestütztes Lernen',
    yongCharacterTitle: 'Die Acht Striche von 永',
    yongCharacterDesc: 'Meistern Sie die acht Grundstriche durch das Zeichen 永 und erschließen Sie das Wesen der chinesischen Kalligrafie',
    yongCharacterStrokes: 'Strichbeherrschung',
  },
  practice: {
    center: 'Zentrum',
    question: 'Frage',
    subtitle: 'Mehrere Übungsmodi, um chinesische Zeichen aus allen Blickwinkeln zu meistern.',
    writingTitle: 'Schreibübung',
    writingDesc: 'Üben Sie das Schreiben auf der intelligenten Leinwand mit Echtzeit-Feedback und KI-Anleitung.',
    quizTitle: 'Gedächtnisquiz',
    quizDesc: 'Lustige Quizze zur Festigung von Zeichenbedeutungen und Aussprachen.',
    progressTitle: 'Lernfortschritt',
    progressDesc: 'Detaillierte Analysen, um Ihre Lernreise und Ihr Wachstum zu verfolgen.',
    startNow: 'Jetzt Starten',
    weeklyProgress: 'Wöchentlicher Fortschritt',
    mon: 'Mo',
    tue: 'Di',
    wed: 'Mi',
    thu: 'Do',
    fri: 'Fr',
    sat: 'Sa',
    sun: 'So',
    today: 'Heute',
    pending: 'Ausstehend',
    charactersLearned: 'Gelernte Zeichen',
    dayStreak: 'Tage Streak',
    accuracy: 'Genauigkeit',
    progress: 'Fortschritt',
    completionRate: 'Vervollständigungsrate',
    status: 'Status',
    allCompleted: 'Alle Abgeschlossen',
    inProgress: 'In Bearbeitung',
    writingDialogDesc: 'Üben Sie das Schreiben chinesischer Zeichen im Raster',
    characterProgress: 'Zeichen {{current}} / {{total}}',
    clear: 'Löschen',
    next: 'Weiter',
    done: 'Fertig',
    quizDialogDesc: 'Wählen Sie die korrekte Pinyin',
    quizComplete: 'Quiz Abgeschlossen!',
    correct: 'Richtig',
    wrong: 'Falsch',
    close: 'Schließen',
    retake: 'Erneut versuchen',
    viewResults: 'Ergebnisse anzeigen',
    listenPronunciation: 'Hören',
    nextQuestion: 'Nächste Frage',
    progressDialogDesc: 'Ihr Lernfortschritt',
    writeOnCanvas: 'Auf der Leinwand schreiben',
  },
  learn: {
    one: 'Eins',
    two: 'Zwei',
    three: 'Drei',
    person: 'Person',
    big: 'Groß',
    small: 'Klein',
    mouth: 'Mund',
    sunDay: 'Sonne/Tag',
    moonMonth: 'Mond/Monat',
    mountain: 'Berg',
    water: 'Wasser',
    fire: 'Feuer',
    radical: 'Radikal',
    structure: 'Struktur',
    independent: 'Unabhängig',
    wordsTitle: 'Wörter/Phrasen',
    exampleTitle: 'Beispiel',
    showStrokeOrder: 'Strichreihenfolge anzeigen',
    hideStrokeOrder: 'Strichreihenfolge ausblenden',
    words: {
      oneDay: 'Ein Tag',
      oneItem: 'Ein Artikel',
      first: 'Erster',
      february: 'Februar',
      second: 'Zweiter',
      twenty: 'Zwanzig',
      march: 'März',
      threeItems: 'Drei Artikel',
      thirty: 'Dreißig',
      people: 'Leute',
      adult: 'Erwachsener',
      chinesePerson: 'Chinese',
      university: 'Universität',
      great: 'Großartig',
      child: 'Kind',
      primarySchool: 'Grundschule',
      careful: 'Vorsichtig',
      saliva: 'Speichel',
      entrance: 'Eingang',
      exit: 'Ausgang',
      days: 'Tage',
      tomorrow: 'Morgen',
      japan: 'Japan',
      moon: 'Mond',
      month: 'Monat',
      firstMonth: 'Erster Monat',
      mountainWater: 'Berge und Flüsse',
      climbMountain: 'Bergsteigen',
      snowMountain: 'Schneeberg',
      fruit: 'Obst',
      riverWater: 'Flusswasser',
      drinkWater: 'Wasser trinken',
      train: 'Zug',
      flame: 'Flamme',
      volcano: 'Vulkan',
    },
    example: {
      oneBook: 'Ich habe ein Buch.',
      twentyYearsOld: 'Er ist dieses Jahr zwanzig Jahre alt.',
      threeApples: 'Es gibt drei Äpfel auf dem Tisch.',
      goodPerson: 'Er ist ein guter Mensch.',
      bigApple: 'Das ist ein großer Apfel.',
      cuteCat: 'Das Kätzchen ist sehr süß.',
      openMouth: 'Öffnen Sie bitte Ihren Mund.',
      goodDay: 'Heute ist ein guter Tag.',
      beautifulMoon: 'Der Mond ist heute Abend schön.',
      climbMountain: 'Ich möchte bergsteigen.',
      glassOfWater: 'Bitte geben Sie mir ein Glas Wasser.',
      carefulFire: 'Vorsicht bei Feuer!',
    },
  },
  footer: {
    copyright: '© 2026 HanziMaster 汉字大师 v3.0.0. Alle Rechte vorbehalten.',
  },
  meta: {
    title: 'HanziMaster 汉字大师',
    description: 'KI-gestützte Plattform zum Erlernen chinesischer Zeichen',
  },
};
