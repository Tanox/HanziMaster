...
interface SearchInputProps {
  ...
  activeChar?: string;
  activeTerm?: string; // New prop to sync full term
}
...
const SearchInput: React.FC<SearchInputProps> = ({ 
  ...
  // Sync input when activeTerm changes (for history navigation)
  useEffect(() => {
    if (activeTerm) {
      setInput(activeTerm);
    } else if (activeChar) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (input.trim() && !isLoading) {
      ...
      // Allow 1 to 4 Chinese characters
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-md mx-auto mb-8">
      <div className="relative flex items-center">
        <input
          ...
          disabled={isLoading}
          maxLength={4} // Allow idioms (usually 4 chars)
        />
