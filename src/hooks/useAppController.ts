...
export const useAppController = () => {
  ...
  // Handle auto-switch to practice mode for continuous mode
  useEffect(() => {
  ...
  const fetchCharacterData = async (char: string, langCode: string) => {
    ...
    if (fetchedData) {
      ...
      setHanziData(null);
      // Only set error if it's the *only* thing we are trying to do (i.e. single char search)
      if (activeTerm.length === 1) {
          setError(`Could not load stroke data for "${char}".`);
      }
    }

    // 2. Fetch AI Analysis for this specific character
  ...
  const handleCharSelect = (char: string) => {
      // Switching tabs in Idiom mode
      if (char === activeChar) return;
      setActiveChar(char);
      setAnimationState(AnimationState.IDLE);
      setInteractionMode(InteractionMode.VIEW);
      // Fetch data for the newly selected char
      // We keep the idiomAnalysis state, but update the single char analysis/strokes
  ...
  const handlePracticeComplete = () => {
    // If practicing an idiom, move to next char
    if (activeTerm.length > 1) {
        ...
        if (currentIndex !== -1 && currentIndex < activeTerm.length - 1) {
            ...
            setTimeout(() => {
                ...
                setInteractionMode(InteractionMode.PRACTICE); // Keep practice mode on
            }, 1000);
            return;
        } else {
             // Idiom finished
    ...
    if (settings.continuousMode) {
      // Need to signal the effect to switch to practice mode after loading next char
