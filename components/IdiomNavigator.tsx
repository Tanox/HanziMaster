...
const IdiomNavigator: React.FC<IdiomNavigatorProps> = ({ term, activeIndex, onSelectChar }) => {
  ...
  return (
    <div 
        id="idiom-navigator-wrapper" 
        className={`flex flex-col items-center gap-3 transition-all duration-500 w-full overflow-hidden ${isIdiomMode ? 'opacity-100 max-h-32 mb-8' : 'opacity-0 max-h-0 pointer-events-none'}`}
    >
       {isIdiomMode && (
           <div className="flex flex-col items-center gap-3 py-1">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-bold tracking-[0.2em]">
                        {activeIndex + 1} / {term.length} • 书写进度
                    </p>
                </div>
           </div>
       )}
