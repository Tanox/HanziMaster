// app/components/analysis/cards/StructureCard.tsx v1.4.4
...
const StructureCard: React.FC<StructureCardProps> = ({ radical, structure, strokeCount, labels }) => (
  <BaseCard icon={<Box size={14} />} title={labels.structure || 'Structure'} className="md:col-span-2">
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-700 pt-2">
        <span className="text-sm text-slate-500 dark:text-slate-400">{labels.strokeCount || 'Strokes'}</span>
        <p className="text-lg font-medium text-slate-700 dark:text-slate-200">{strokeCount}</p>
      </div>
      {structure && (
        <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-700 pt-2">
