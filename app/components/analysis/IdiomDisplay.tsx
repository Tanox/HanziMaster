// app/components/analysis/IdiomDisplay.tsx v1.1.6
...
const IdiomDisplay: React.FC<IdiomDisplayProps> = ({ data, labels }) => {
  ...
  const shareUrl = `${origin}?char=${encodeURIComponent(data.idiom)}`;
        ...
        <div className="relative z-10">
            ...
            <div className="grid md:grid-cols-2 gap-6 pt-4 border-t border-slate-100 dark:border-slate-700">
                <div className="space-y-2">
                     <div className="flex items-center gap-2 text-amber-600 dark:text-amber-500"><ScrollText size={18} /><h3 className="font-bold text-sm uppercase tracking-wide">{labels.idiomOrigin || labels.origin}</h3></div>
                     <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed">{data.origin}</p>
                </div>
                <div className="space-y-2">
                     <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400"><Quote size={18} /><h3 className="font-bold text-sm uppercase tracking-wide">{labels.idiomUsage || 'Usage'}</h3></div>
                     <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed italic bg-slate-50 dark:bg-slate-900/50 p-3 rounded-lg border border-slate-100 dark:border-slate-700/50">{data.usage}</p>
                </div>
            </div>
        </div>
    </div>
