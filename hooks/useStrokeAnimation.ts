...
export const useStrokeAnimation = (
  ...
  // Pre-calculate stroke lengths for smooth animation timing
  const strokeLengths = useMemo(() => {
    if (!data || !data.medians) return [];
    return data.medians.map(getPathLength);
  }, [data]);

  // Reset state when data or mode changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
        ...
        // Base duration calculation based on length and speed
        // 800 units length = 1 second at 1x speed
